...
## 7. Early Returns and Guard Clauses

### ✅ DO
- Use early returns (guard clauses) at the very beginning of the method to validate parameters and preconditions.
- Keep the main business logic unnested after the validations.
- Ensure the method has a single exit point after the initial guard clauses.
...
### ❌ DON'T
- Use multiple `return` statements scattered throughout the business logic.
- Use early `return` statements inside loops (`for`, `while`) or control statements (`switch`, `if` representing business logic).
- Create nested if-else pyramids (pyramid of doom) for validation.
...
### ✅ DO
- Format all Java code using **Google Java Format**.
- Ensure consistent indentation, line wrapping, and import ordering as defined by the formatter.
- Run the formatter before committing code.

### ❌ DON'T
- Submit code with inconsistent formatting.
- Argue about style details in code reviews; let the formatter decide.
...
- [ ] Early returns used only for initial validations
